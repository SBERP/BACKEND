...
class EncodeAllData extends ClientService
{
	public function getEncodedAllData($status)
	{
		...
		for($decodedData=0;$decodedData<count($deocodedJsonData);$decodedData++)
		{
			...
			// $architectId = $deocodedJsonData[$decodedData]->architect_id;
			// $carpenterId = $deocodedJsonData[$decodedData]->carpenter_id;
			// $clientArchitectId = $deocodedJsonData[$decodedData]->client_architect_id;
			// $clientCarpenterId = $deocodedJsonData[$decodedData]->client_carpenter_id;

			$architectCommission = $deocodedJsonData[$decodedData]->architect_commission;
			$carpenterCommission = $deocodedJsonData[$decodedData]->carpenter_commission;
			$clientArchitectCommission = $deocodedJsonData[$decodedData]->client_architect_commission;
			$clientCarpenterCommission = $deocodedJsonData[$decodedData]->client_carpenter_commission;

			$architectCFlag = 0;
			$carpenterCFlag = 0;

			if($architectCommission==$clientArchitectCommission)
			{
				$architectCFlag = 0;
			}
			else if(($architectCommission==0) && ($clientArchitectCommission!=0))
			{
				$architectCFlag = 1;
			}
			else
			...
			if($carpenterCommission==$clientCarpenterCommission)
			{
				$carpenterCFlag = 0;
			}
			else if(($carpenterCommission==0) && ($clientCarpenterCommission!=0))
			{
				$carpenterCFlag = 1;
			}
			else
			...
			if($architectCFlag!=0)
				$data[$decodedData]['architectCommission'] = (($architectCFlag==1)?false:true);
			if($carpenterCFlag!=0)
				$data[$decodedData]['carpenterCommission'] = (($carpenterCFlag==1)?false:true);
