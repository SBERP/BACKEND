...
class BillModel extends Model
{
	...
	*/
	...
	public function updateACCommission($jfId,$clientId,$amount)
	{
		...
		DB::beginTransaction();
		$raw = DB::connection($databaseName)->select("select carpenter_id,carpenter_commission,architect_id,architect_commission from client_mst where client_id='".$clientId."'");
		DB::commit();

		DB::beginTransaction();
			$celraw = DB::connection($databaseName)->select("select max(ledger_id) as ledger_id from ledger_mst where ledger_name='commission(expense)'");
		DB::commit();


		if(count($raw)==0 || count($celraw)==0)
			return [];

		if($raw[0]->carpenter_id!=null && $raw[0]->carpenter_commission!=null && $raw[0]->carpenter_id!=0 && $raw[0]->carpenter_commission!=0)
		{
			$camount = $amount*($raw[0]->carpenter_commission/100);

			DB::beginTransaction();
			$clraw = DB::connection($databaseName)->select("select ledger_id from ledger_mst where staff_id='".$raw[0]->carpenter_id."'");
			DB::commit();
			...
			{
				DB::beginTransaction();
				$oldcfraw = DB::connection($databaseName)->select("select * from ".$clraw[0]->ledger_id."_ledger_dtl where jf_id=".$jfId);
				DB::commit();

				if(count($oldcfraw)>0)
				{
					DB::beginTransaction();
					$cfraw = DB::connection($databaseName)->statement("update ".$clraw[0]->ledger_id."_ledger_dtl set amount='".$camount."',amount_type='credit',entry_date='".$mytime."',updated_at='".$mytime."' where jf_id=".$jfId);
					DB::commit();
				}
				else
				{
					DB::beginTransaction();
					$cfraw = DB::connection($databaseName)->statement("insert into ".$clraw[0]->ledger_id."_ledger_dtl(amount,amount_type,entry_date,jf_id,ledger_id,created_at) 
					values(".$camount.",'credit','".$mytime."',".$jfId.",".$celraw[0]->ledger_id.",'".$mytime."')");
					DB::commit();
				}
				DB::beginTransaction();
				$oldcfraw = DB::connection($databaseName)->select("select * from ".$celraw[0]->ledger_id."_ledger_dtl where jf_id=".$jfId." and ledger_id=".$clraw[0]->ledger_id);
				DB::commit();

				if(count($oldcfraw)>0)
				{
					DB::beginTransaction();
					$cfraw = DB::connection($databaseName)->statement("update ".$celraw[0]->ledger_id."_ledger_dtl set amount='".$camount."',amount_type='credit',entry_date='".$mytime."',updated_at='".$mytime."' where jf_id=".$jfId);
					DB::commit();
				}
				else
				{
					DB::beginTransaction();
					$cfraw = DB::connection($databaseName)->statement("insert into ".$celraw[0]->ledger_id."_ledger_dtl(amount,amount_type,entry_date,jf_id,ledger_id,created_at) 
					values(".$camount.",'debit','".$mytime."',".$jfId.",".$clraw[0]->ledger_id.",'".$mytime."')");
					DB::commit();
				}
			}
		}
		if($raw[0]->architect_id!=null && $raw[0]->architect_commission!=null && $raw[0]->architect_id!=0 && $raw[0]->architect_commission!=0 )
		{
			$aamount = $amount*($raw[0]->architect_commission/100);

			DB::beginTransaction();
			$alraw = DB::connection($databaseName)->select("select ledger_id from ledger_mst where staff_id='".$raw[0]->architect_id."'");
			DB::commit();

			if(count($alraw)>0)
			{
				DB::beginTransaction();
				$oldaf = DB::connection($databaseName)->select("select * from ".$alraw[0]->ledger_id."_ledger_dtl where jf_id=".$jfId);
				DB::commit();

				if(count($oldaf)>0)
				{
					DB::beginTransaction();
					$af = DB::connection($databaseName)->statement("update ".$alraw[0]->ledger_id."_ledger_dtl set amount='".$aamount."',amount_type='credit',entry_date='".$mytime."',updated_at='".$mytime."' where jf_id=".$jfId);
					DB::commit();
				}
				else
				{
					DB::beginTransaction();
					$af = DB::connection($databaseName)->statement("insert into ".$alraw[0]->ledger_id."_ledger_dtl(amount,amount_type,entry_date,jf_id,ledger_id,created_at) 
					values(".$aamount.",'credit','".$mytime."',".$jfId.",".$celraw[0]->ledger_id.",'".$mytime."')");
					DB::commit();
				}
				DB::beginTransaction();
				$oldaf = DB::connection($databaseName)->select("select * from ".$celraw[0]->ledger_id."_ledger_dtl where jf_id=".$jfId." and ledger_id=".$alraw[0]->ledger_id);
				DB::commit();

				if(count($oldaf)>0)
				{
					DB::beginTransaction();
					$af = DB::connection($databaseName)->statement("update ".$celraw[0]->ledger_id."_ledger_dtl set amount='".$aamount."',amount_type='credit',entry_date='".$mytime."',updated_at='".$mytime."' where jf_id=".$jfId);
					DB::commit();
				}
				else
				{
					DB::beginTransaction();
					$cfraw = DB::connection($databaseName)->statement("insert into ".$celraw[0]->ledger_id."_ledger_dtl(amount,amount_type,entry_date,jf_id,ledger_id,created_at) 
					values(".$aamount.",'debit','".$mytime."',".$jfId.",".$alraw[0]->ledger_id.",'".$mytime."')");
					DB::commit();
				}
			}
		}
	...
	public function insertCarpenterArchitectCommission($jfId,$clientId,$amount)
	{
		$mytime = Carbon\Carbon::now();
		...
		if($raw[0]->architect_id!=null && $raw[0]->architect_commission!=null && $raw[0]->architect_id!=0 && $raw[0]->architect_commission!=0 )
		{
			...
			if(count($alraw)>0)
			{
				...
				DB::beginTransaction();
				$af = DB::connection($databaseName)->statement("insert into ".$celraw[0]->ledger_id."_ledger_dtl(amount,amount_type,entry_date,jf_id,ledger_id,created_at) 
				values(".$aamount.",'debit','".$mytime."',".$jfId.",".$alraw[0]->ledger_id.",'".$mytime."')");
				DB::commit();
			}
		}
	...
	public function insertAllData($productArray,$paymentMode,$bankLedgerId,$invoiceNumber,$jobCardNumber,$bankName,$checkNumber,$total,$extraCharge,$tax,$grandTotal,$advance,$balance,$remark,$entryDate,$companyId,$branchId,$ClientId,$salesType,$documentArray,$jfId,$totalDiscounttype,$totalDiscount,$totalCgstPercentage,$totalSgstPercentage,$totalIgstPercentage,$poNumber,$requestData,$expense,$serviceDate,$userId,$createdBy = 0)
	{
		...
		$architectIdVal = 0;
		$architectCommissionVal = 0;
		$carpenterIdVal = 0;
		$carpenterCommissionVal = 0;
		...
		if(count($raw)>0)
		{
			$architectCommissionVal=$raw[0]->architect_commission;
			$carpenterCommissionVal=$raw[0]->carpenter_commission;
			$architectIdVal = $raw[0]->architect_id;
			$carpenterIdVal = $raw[0]->carpenter_id;
		}
		$expanseVal = 0; 
		if(!empty(json_decode($expense)))
		{
			
			$expanseVal = json_decode($expense)[0]->expenseValue;
		}
		$mytime = Carbon\Carbon::now();
		//database selection
		...
		if(array_key_exists("isDraft",$requestInput))
		{
			...
			architect_id='".$architectIdVal."',
			carpenter_id='".$carpenterIdVal."',
			architect_commission='".$architectCommissionVal."',
			carpenter_commission='".$carpenterCommissionVal."',
			client_id='".$ClientId."',
			jf_id='".$jfId."',
			dispatch_status=0,
			...
			{
				$caReturn = $this->insertCarpenterArchitectCommission($jfId,$ClientId,($total-$expanseVal));	
	...
	public function insertData($productArray,$paymentMode,$bankLedgerId,$invoiceNumber,$jobCardNumber,$bankName,$checkNumber,$total,$extraCharge,$tax,$grandTotal,$advance,$balance,$remark,$entryDate,$companyId,$branchId,$ClientId,$salesType,$jfId,$totalDiscounttype,$totalDiscount,$totalCgstPercentage,$totalSgstPercentage,$totalIgstPercentage,$poNumber,$requestData,$expense,$serviceDate,$userId,$createdBy = 0)
	{
		...
		{			
			...
			if(count($getJobCardNumber)==0)
			{
				...
				architect_commission,
				carpenter_commission) 
				values({$valueStr},'".$architectIdVal."','".$carpenterIdVal."','".$architectCommissionVal."','".$carpenterCommissionVal."')", $valueArray);
	...
	*/
	public function insertBillDraftData(Request $request)
	{
		...
		if(array_key_exists("saleid",$request->header()))
		{
			...
			set ".$updateString."updated_at='".$mytime."',is_draft='yes',product_array='".$inventoryDecodedData."',dispatch_status=0 
	...
	*/
	...
	public function getQuotationBasedData($companyId,$data)
	{
		//database selection
		$database = "";
		$constantDatabase = new ConstantClass();
		...
		if(is_object($data))
		{
			...
			for($i=0;$i<=10;$i++)
			{
				...
				s.architect_id,
				s.carpenter_id,
				s.architect_commission,
				s.carpenter_commission,
				cli.architect_id as client_architect_id,
				cli.architect_commission as client_architect_commission,
				cli.carpenter_id as client_carpenter_id,
				cli.carpenter_commission as client_carpenter_commission,
				e.expense,
				d.file
				...
				LEFT JOIN client_mst as cli on cli.client_id=s.client_id 

				where s.sales_type='".$salesType."' and
				(s.entry_date BETWEEN '".$fromDate."' AND '".$toDate."') and 
				s.company_id='".$companyId."' and 
				s.deleted_at='0000-00-00 00:00:00' and s.is_draft='no' and ".$isSalesOrder.$branch_check." and
				s.dispatch_status=".$i);
				DB::commit();
				$temp = [];
				$temp['dispatch_'.$i] = $raw;
				$data[] = $temp;
			}
			if(count($data)==0)
			{
	...
	*/
	...
	public function getBillById($id)
	{
		...
		LEFT JOIN client_mst as cli on cli.client_id=s.client_id 
		where s.sale_id = '$id' and
				s.deleted_at='0000-00-00 00:00:00'");
		DB::commit();
		return json_encode($raw);
	}

	public function getBillMonthwise()
	{
		//database selection
		$database = "";
		$constantDatabase = new ConstantClass();
		...
		DB::statement('SET group_concat_max_len = 1000000');
		$raw = $this->database->select("SELECT 
		concat(YEAR(service_date),'-',MONTH(service_date)) as Month,
		sum(total) as Total,
		...
		sum(refund) as Refund  FROM `sales_bill` GROUP BY MONTH(service_date), YEAR(service_date)");
		DB::commit();
		foreach($raw as $data)
		{
			...
			$data->sale = $this->database->select("SELECT 
				s.sale_id,
				s.product_array,
				s.payment_mode,
				s.bank_ledger_id,
				...
				LEFT JOIN client_mst as cli on cli.client_id=s.client_id 
				where MONTH(s.service_date)=$month and YEAR(s.service_date)=$year and s.deleted_at='0000-00-00 00:00:00'");
				DB::commit();
		}
		return json_encode($raw);
	}
	...
	*/
	public function updateBillData($billArray,$saleId,$documentArray,$headerData)
	{
		...
		/**************************Update for commission update*************************/
		// $newAmount = 0;
		if(array_key_exists("architect_commission",$billArray) && array_key_exists("carpenter_commission",$billArray))
		if($billArray['architect_commission']=="true" || $billArray['carpenter_commission']=="true")
		{
			DB::beginTransaction();
			$expenseValueDB = $this->database->select("select expense_value 
				from sale_expense_dtl 
				where sale_id = ".$saleId." and
			deleted_at='0000-00-00 00:00:00' order by sale_expense_id DESC");
			DB::commit();
			$expenseOLD_VAL = 0;
			if(count($expenseValueDB)>0)
			{
				$expenseOLD_VAL = $expenseValueDB[0]->expense_value;
			}
			DB::beginTransaction();
			$saleValueDB = $this->database->select("select total,client_id,jf_id 
				from sales_bill 
				where sale_id = ".$saleId." and
			deleted_at='0000-00-00 00:00:00'");
			DB::commit();
			...
			$JFDetailId = 0;
			$newAmount = 0;
			if(count($saleValueDB))
			{
				$newAmount = $saleValueDB[0]->total;
				$salesOLD_VAL = $saleValueDB[0]->total;
				$clientDetailId = $saleValueDB[0]->client_id;
				$JFDetailId = $saleValueDB[0]->jf_id;
			}
			$oldAmount = $salesOLD_VAL - $expenseOLD_VAL;

			$expenseVal_NEW = 0;
			if(array_key_exists('expense',$billArray))
			{
				$expenseVal_NEW = json_decode($billArray['expense'])[0]->expenseValue;
			}
			if(array_key_exists('total', $billArray))
			{
				$newAmount = $billArray['total'] - $expenseVal_NEW;
			}
			else
			{
				$newAmount = $oldAmount;
			...
			// {
				// $this->updateACCommission($JFDetailId,$clientDetailId,$newAmount);
				// return json_encode(['amount same',$oldAmount,$newAmount]);
			// }
		}
		...
		{
			DB::beginTransaction();
			$raw = DB::connection($databaseName)->select("select carpenter_id,carpenter_commission,architect_id,architect_commission from client_mst where client_id=(select client_id from sales_bill where sale_id=".$saleId.")");
			DB::commit();
			if(count($raw)>0)
			{
				$valueArray[0]=$raw[0]->architect_commission;
				$valueArray[1]=$raw[0]->carpenter_commission;
				array_push($valueArray, $raw[0]->architect_id);
				array_push($valueArray, $raw[0]->carpenter_id);
				$keyValueString.="architect_id= ?,carpenter_id= ?,";
			}
			else
			{
				$keyValueString = implode(',', array_slice(explode(',', $keyValueString),2));
				$valueArray = array_slice($valueArray,2);
			}
		}
		else
		{
		...
		if(($billArray['architect_commission']=="true" || $billArray['carpenter_commission']=="true"))
		{
			...
			// $newAmount = $billArray['total'] - $expenseVal_NEW;
			// return json_encode([$JFDetailId,$clientDetailId,$newAmount]);
			$this->updateACCommission($JFDetailId,$clientDetailId,$newAmount);
		}
		...
		// DB::beginTransaction();
		// $rawaaa = $this->database->select("select ledger_id from ledger_mst where client_id='".$saleValueDB[0]->client_id."'");
		// DB::commit();

		// DB::beginTransaction();
		// $saledetaileddata = $this->database->select("select max(ledger_id) as ledger_id from ledger_mst where ledger_name='whole_sales'");
		// DB::commit();

		// if(count($rawaaa)>0 && count($saledetaileddata)>0)
		// {
		// 	DB::beginTransaction();
		// 	$raw = $this->database->select("update ".$raw[0]->ledger_id."_ledger_dtl set ledger_id='".$saledetaileddata[0]->ledger_id."' where jf_id=".$saleValueDB[0]->jf_id);
		// 	DB::commit();
		// }
