...
class PurchaseBillModel extends Model
{
	...
	*/
	...
	public function getPurchaseBillMonthwise()
	{
		...
		concat(YEAR(entry_date),'-',MONTH(entry_date)) as Month,
		sum(total) as Total,
		...
		sum(balance) as Balance  FROM `purchase_bill` GROUP BY MONTH(entry_date), YEAR(entry_date)");
		DB::commit();
		foreach($purchaseData as $data)
		{
			...
			$data->purchase =  $this->database->select("
				...
				where p.bill_type='purchase_bill' and MONTH(entry_date)=$month and YEAR(entry_date)=$year and
